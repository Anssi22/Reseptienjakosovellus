<script>
  ...
  .filter(Boolean)
  .map((row) => {
      const [nimi, maara, yksikko] = row.split(";").map((x) => (x ?? "").trim());
    ...
    async function lisaaResepti() {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(`Lisäys epäonnistui (${res.status}): ${robleemaviesti}`);
            }
...
<section class="grid">
  ...
  <div class="card">
    ...
    <div class="row">
      <input placeholder="esim. kana,sipuli" bind:value={ainesosaHaku} />
        <button class="btn btnPrimary" type="button" on:click={lataaReseptit} disabled={loading}>
        {loading ? "Haetaan..." : "Hae"}
        </button>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <form on:submit|preventDefault={lisaaResepti}>
      ...
      <textarea rows="5" placeholder="Reseptin ohjeet" bind:value={ohjeet}></textarea>
      ...
      ></textarea>
        ...
        <button class="btn btnPrimary" type="submit" disabled={!otsikko || !ohjeet}>
        Lisää resepti
        </button>
    </form>
  </div>
    ...
    {:else}
        ...
        {#each reseptit as r (r._id)}
            ...
            <div class="topRow">
                ...
                <div class="rating">
                Tähdet {r.arvioKeskiarvo ? r.arvioKeskiarvo.toFixed(1) : "0.0"} / 5
                ({r.arvioMaara ?? 0})
                </div>
            </div>
            ...
            <div class="row actions">
                <button class="btn btnPrimary" type="button" on:click={() => toggleAuki(r._id)}>
                    {aukiId === r._id ? "Piilota" : "Näytä"}
                </button>

                <button class="btn btnSecondary" type="button" on:click={() => aloitaMuokkaus(r)} disabled={loading}>
                    Muokkaa
                </button>

                <button class="btn btnDanger" type="button" on:click={() => poistaResepti(r._id)} disabled={loading}>
                    Poista
                </button>
            </div>
            ...
            {#if aukiId === r._id}
                ...
                <div class="detailsSection">
                    ...
                    {r.ainesosat?.length ? r.ainesosat.map((a) => a.nimi).join(", ") : "-"}
                    </div>
                </div>
                ...
                <div class="detailsSection addSection">
                    ...
                    <select bind:value={arvioDraft[r._id]}>
                        ...
                        <button class="btn btnPrimary" type="button" on:click={() => lisaaArvio(r._id)}>Anna arvio</button>
                    </div>
                    ...
                    />
                    <button class="btn btnPrimary" type="button" on:click={() => lisaaKommentti(r._id)}>Lisää kommentti</button>
                    </div>
                </div>
                </div>
            {/if}
            ...
            {#if muokattavaId === r._id}
                ...
                <textarea rows="4" placeholder="Ohjeet" bind:value={editOhjeet}></textarea>
                <textarea
                    rows="5"
                ...
                <div class="row">
                    ...
                    <button class="btn btnDanger" type="button" on:click={peruMuokkaus}>Peru</button>
                </div>
                </form>
            {/if}
            </li>
...
.btn {
  ...
  font-weight: 700;
  cursor: pointer;
  transition: transform 0.02s ease, opacity 0.15s ease, background 0.15s ease, border-color 0.15s ease;
}
